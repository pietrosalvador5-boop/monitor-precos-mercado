...
async def abrir_busca(page, loja_cfg, termo):
    if loja_cfg.get("busca_url"):
        url = loja_cfg["busca_url"].format(query=quote_plus(termo))
        ...
        # IMPORTANTE: a ordem importa. 'Pesquis' (com i = case-insensitive,
        # cobre "Pesquisar" e "pesquisar") é o que funciona comprovadamente no
        # site do Zaffari, então vem primeiro. NÃO deixamos um seletor "input"
        # genérico no final: se nenhum seletor específico funcionar, é melhor
        # falhar com erro claro do que digitar o termo de busca em um campo
        # errado (CEP, login, newsletter etc.) e devolver um resultado
        ...
        for sel in possiveis:
            try:
                ...
                continue
        if campo is None:
            raise RuntimeError("Campo de busca não encontrado no site (nenhum seletor conhecido bateu).")
        url_antes = page.url
        await campo.click(timeout=10000)
        await campo.fill(termo, timeout=10000)
        await campo.press("Enter", timeout=10000)

        # O banner de cookies pode aparecer só DEPOIS da busca (não só antes),
        # então tentamos fechá-lo de novo aqui — se ficar sobreposto na tela,
        # pode interferir em cliques subsequentes.
        await aceitar_cookies(page)

        # Espera a navegação de fato acontecer: a URL deve mudar (ex: vai para
        # /arroz-5kg?_q=...). Só esperar um tempo fixo não garante que a busca
        # processou — em alguns casos a página continua na home (carrossel)
        # mesmo depois do Enter, e o robô acaba raspando produtos errados.
        try:
            await page.wait_for_url(lambda u: u != url_antes, timeout=10000)
        except Exception:
            # Se a URL não mudou em 10s, ainda pode ser uma busca "ajax" que
            # troca o conteúdo sem mudar a URL. Não falha aqui; deixa o
            # extrair_cards() tentar mesmo assim, mas isso é candidato a
            # 'nao_encontrado'/'revisar_match' caso não haja card de produto.
            pass
